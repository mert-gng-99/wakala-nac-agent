# Wakala - Prototype Phase submission
...
**YOU.** Pick your Idea Phase submission for Wakala from the dropdown.
...
```
1. Trusted Digital Identity & Cross-Border Verification
```
...
1. The dashboard after Run all seven acts, showing the batch table and the cost comparison.
...
5 CAMARA APIs, 42.9% cheaper than calling every check.
...
```
https://github.com/mert-gng-99/wakala-nac-agent
```
...
| --- | --- | --- |
| Parent Submission | Pick Wakala from the dropdown | seconds |
...
| --- | --- |
| At least one CAMARA API on Nokia NaC | 5 API families: `number-verification`, `sim-swap`, `device-swap`, `device-status`, `location-verification` |
...
| Solves a real problem | Trusted Digital Identity & Cross-Border Verification. Stated limits are in README.md, so this is not pitched as doing more than it does. |
| Theme alignment | Theme 1 |
| Multiple CAMARA APIs (good-to-have) | 5 families orchestrated per case |
| Intelligent orchestration (good-to-have) | 1 to 7 calls depending on the case; 42.9% cheaper than calling everything |
